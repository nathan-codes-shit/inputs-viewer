...
void BackgroundSprite::addBackgroundNode(CCNode* node) {
    if (auto protocol = typeinfo_cast<CCRGBAProtocol*>(node)) {
        protocol->setColor(ccc3(m_backgroundColor.get().r, m_backgroundColor.get().g, m_backgroundColor.get().b));
        protocol->setOpacity(m_backgroundColor.get().a);
        if (auto s9p = typeinfo_cast<CCScale9Sprite*>(node)) {
            s9p->setColor(ccc3(m_backgroundColor.get().r, m_backgroundColor.get().g, m_backgroundColor.get().b));
            s9p->setOpacity(m_backgroundColor.get().a);
        }
...
void BackgroundSprite::addOutlineNode(CCNode* node) {
    if (auto protocol = typeinfo_cast<CCRGBAProtocol*>(node)) {
        protocol->setColor(ccc3(m_outlineColor.get().r, m_outlineColor.get().g, m_outlineColor.get().b));
        protocol->setOpacity(m_outlineColor.get().a);
        if (auto s9p = typeinfo_cast<CCScale9Sprite*>(node)) {
            s9p->setColor(ccc3(m_outlineColor.get().r, m_outlineColor.get().g, m_outlineColor.get().b));
            s9p->setOpacity(m_outlineColor.get().a);
        }
...
void BackgroundSprite::addTextNode(CCNode* node) {
    if (auto protocol = typeinfo_cast<CCRGBAProtocol*>(node)) {
        protocol->setColor(ccc3(m_textColor.get().r, m_textColor.get().g, m_textColor.get().b));
        protocol->setOpacity(m_textColor.get().a);
        if (auto s9p = typeinfo_cast<CCScale9Sprite*>(node)) {
            s9p->setColor(ccc3(m_textColor.get().r, m_textColor.get().g, m_textColor.get().b));
            s9p->setOpacity(m_textColor.get().a);
        }
