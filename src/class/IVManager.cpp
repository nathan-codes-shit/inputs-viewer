...
Result<inputs_viewer::LevelSettings> matjson::Serialize<inputs_viewer::LevelSettings>::fromJson(matjson::Value const& value) {
    inputs_viewer::LevelSettings settings;
    settings.p1Transform = value["p1-display"].as<NodeTransform>().unwrapOr(inputs_viewer::IVManager::getDefaultP1Transform());
    settings.p2Transform = value["p2-display"].as<NodeTransform>().unwrapOr(inputs_viewer::IVManager::getDefaultP2Transform());
    settings.showTotalInputs = value["show-total-inputs"].asBool().unwrapOr(false);
    settings.showCPS = value["show-cps"].asBool().unwrapOr(false);
    settings.hideLeftRight = value["hide-left-right"].asBool().unwrapOr(false);
    return geode::Ok(settings);
}