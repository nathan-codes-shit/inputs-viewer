...
SettingsLayer* SettingsLayer::create(LevelSettingsType levelType, bool enableGeodeSettingButton) {
    ...
    if (ret && ret->init(350.f, 260.f, "square02_001.png") && ret->setup(enableGeodeSettingButton)) {
        ret->autorelease();
        return ret;
    }
    delete ret;
...
bool SettingsLayer::setup(bool enableGeodeSettingButton) {
    ...
    m_totalInputsCheckbox = this->createCheckbox(&LevelSettings::showTotalInputs, "Show Total Inputs", Anchor::Center, ccp(-90.f, -50.f));
    m_cpsCheckbox = this->createCheckbox(&LevelSettings::showCPS, "Show Clicks per Seconds", Anchor::Center, ccp(-90.f, -80.f));
    m_hideLRCheckbox = this->createCheckbox(
        &LevelSettings::hideLeftRight, "Hide L&R Keys", Anchor::Center, ccp(-90.f, -110.f),
...
CCMenuItemToggler* SettingsLayer::createCheckbox(bool LevelSettings::* member, char const* text, Anchor anchor, CCPoint const& offset, char const* description) {
    auto checkbox = CCMenuItemExt::createTogglerWithStandardSprites(0.7f, [this, member](CCMenuItemToggler* btn) {
        m_currentSetting.get().*member = !btn->isToggled();
        m_inputsLayer->refreshDisplay();
    });
